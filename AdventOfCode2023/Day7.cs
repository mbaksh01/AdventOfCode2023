...
public class Day7
{
    ...
    public static int Part2(string[] rows)
    {
        foreach (string row in rows)
        {
            SetBidAndGroupPart2(row);
        }

        foreach (Group group in _cardGroups.Keys)
        {
            OrderGroupPart2(group);
        }
    ...
    private static void SetBidAndGroupPart2(string row)
    {
        string[] parts = row.Split(' ');
        string hand = parts[0];
        ...
        if (hand.Contains('J'))
        {
            ...
            Console.WriteLine($"Replacing J with {key}.");
            ...
            Console.WriteLine($"{hand.Replace('J', key)}");
        }
        ...
        if (handValues.Count == 3)
        {
            if (handValues[0].Count() == 3)
            {
                ...
                {
                    _cardGroups[Group.ThreeOfAKind].Add(hand);
                    Console.WriteLine("ThreeOfAKind");
                    return;
                }
            }
    ...
    private static void OrderGroupPart2(Group group)
    {
        List<string> hands = _cardGroups[group];
        ...
        do
        {
            ...
            for (int i = 0; i < hands.Count - 1; i++)
            {
                if (IsBiggerPart2(hands[i], hands[i + 1]))
                {
                    (hands[i], hands[i + 1]) = (hands[i + 1], hands[i]);
                    swapped = true;
                }
    ...
    private static bool IsBiggerPart2(string hand1, string hand2)
    {
        List<char> order = new()
